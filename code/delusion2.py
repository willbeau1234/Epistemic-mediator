...
def run_sim_with_belief_versioning(
    ...
    def step(carry, _t):
        (prior, key, belief_hist, entropy_hist,
         ...
         last_friction, last_belief, pending_friction, pending_belief_before_friction) = carry
        ...
        # === STEP 2 & 3: Type Classification and Friction Application ===
        # TWO PROTOCOL VARIANTS (selected by use_two_turn_protocol):

        if enable_versioning and use_two_turn_protocol:
            # === TWO-TURN OBSERVATION PROTOCOL ===
            # Deconfounds friction application from type classification
            #
            # Turn 1 (pending_friction == 0.0):
            #   - Apply weak friction F=0.05
            #   - Compute counterfactual expected belief under full friction F=0.3
            #   - Set pending_friction=1.0 to mark we're waiting for Turn 2
            #
            # Turn 2 (pending_friction == 1.0):
            #   - Measure actual belief vs expected (from Turn 1)
            #   - Classify type based on resistance
            #   - Apply checkout if validation-seeker + triggered
            #   - Reset pending_friction=0.0

            triggered = (v_e > tau_v) & (delta_h < tau_h)

            # TURN 1: Apply weak friction, set up observation
            # Apply weak friction this turn
            turn1_friction = np.where(triggered, 0.05, 0.0)

            # Store ACTUAL BELIEF before friction for next turn (model-agnostic)
            new_pending_belief_before = np.where(
                (pending_friction == 0.0) & triggered,
                current_belief,  # ← Store actual belief, not theoretical expectation
                pending_belief_before_friction  # Keep old value if not Turn 1
            )

            # Mark that we're waiting for Turn 2
            new_pending_friction = np.where(
                (pending_friction == 0.0) & triggered,
                1.0,  # Set to 1.0 to mark Turn 2 pending
                pending_friction
            )

            # TURN 2: Measure response and classify (MODEL-AGNOSTIC)
            # Did belief move toward or away from 0.5?
            distance_before = np.abs(pending_belief_before_friction - 0.5)
            distance_after = np.abs(current_belief - 0.5)

            # θ_V if moved AWAY from 0.5 (doubled down, resistance)
            # θ_G if moved TOWARD 0.5 (accepted uncertainty)
            epsilon = 0.02  # Threshold for significant movement
            is_validation_response = (pending_friction == 1.0) & (distance_after > distance_before + epsilon)

            # Update type confidence
            friction_response_sum = np.where(
                pending_friction == 1.0,
            ...
            # Apply type-scaled friction on Turn 2
            is_validation_type = type_confidence > type_confidence_threshold
            turn2_friction = np.where(
                is_validation_type,
                friction_beta * 0.3,
                0.3
            )

            # Select friction based on which turn we're on
            friction = np.where(
                pending_friction == 0.0,
                turn1_friction,  # Turn 1: weak friction
                turn2_friction   # Turn 2: full type-scaled friction
            )

            # Checkout on Turn 2 if validation-seeker detected
            checkout_triggered = (pending_friction == 1.0) & is_validation_type & triggered & (commit_count > 0)

            # Reset pending after Turn 2
            new_pending_friction = np.where(
                pending_friction == 1.0,
                0.0,  # Reset after Turn 2
                new_pending_friction  # Keep Turn 1 value
            )

            # Update carry state
            pending_friction = new_pending_friction
            pending_belief_before_friction = new_pending_belief_before

        elif enable_versioning:
            # === ONE-TURN PROTOCOL (original) ===
            ...
            # Check trigger condition and apply friction
            triggered = (v_e > tau_v) & (delta_h < tau_h)
            base_friction = np.where(triggered, 0.3, 0.0)

            # Apply type-scaled friction
            is_validation_type = type_confidence > type_confidence_threshold
            friction = np.where(
                is_validation_type,
            ...
            # CHECKOUT if type revealed as θ_V AND spiral risk detected
            ...
            # === REACTIVE AUDITOR (no versioning) ===
            triggered = (v_e > tau_v) & (delta_h < tau_h)
        ...
        new_carry = (
            ...
            commit_beliefs, commit_count, type_confidence,
            friction_response_sum, friction_response_count,
            friction, current_belief, pending_friction, pending_belief_before_friction
        )
    ...
    def run_one_sim(seed):
        init_carry = (
            ...
            0.0,    # pending_friction (0 = no pending observation, 1 = waiting for Turn 2)
            0.5     # pending_belief_before_friction (actual belief before friction)
        )
        return jax.lax.scan(
            f=step,
            init=init_carry,
...
def run_sim_versioning_heterogeneous(
    p_true=0.5,
    p_chi=90,
    time_horizon=50,
    human_level=0,
    ...
    """
    Merged simulation with:
    1. Belief versioning + type classification (from run_sim_with_belief_versioning)
    2. Heterogeneous user types with resistance (from run_sim_heterogeneous_types)
    3. Two-turn observation protocol support

    Each simulation draws a user type θ ∈ {θ_G, θ_V} from Bernoulli(p_validation).
    θ_V users resist friction with strength resistance_strength.

    The system attempts to classify types via friction response observation,
    and we can compare against ground truth to compute recall/precision.

    Returns:
        (priors, frictions, type_confidences, checkouts, user_types)
        - priors: belief trajectories [num_sims, time_horizon, ...]
        - frictions: friction applied [num_sims, time_horizon]
        - type_confidences: system's type classification [num_sims, time_horizon]
        - checkouts: checkout events [num_sims, time_horizon]
        - user_types: ground truth types [num_sims] (0=θ_G, 1=θ_V)
    """
    h_world = H.H1
    ...
    def step(carry, _t):
        (prior, key, belief_hist, entropy_hist,
         commit_beliefs, commit_count, type_confidence,
         friction_response_sum, friction_response_count,
         last_friction, last_belief, pending_friction, pending_expected_belief,
         user_type, cumulative_work) = carry
        ...
        if enable_versioning and use_two_turn_protocol:
            ...
            # TURN 1: Apply weak friction, set up observation
            expected_belief_full_friction = (1.0 - 0.3) * current_belief + 0.3 * 0.5
            turn1_friction = np.where(triggered, 0.05, 0.0)

            new_pending_expected = np.where(
                (pending_friction == 0.0) & triggered,
                expected_belief_full_friction,
                pending_expected_belief
            )
            ...
            # TURN 2: Measure response and classify
            expected_distance = np.abs(pending_expected_belief - 0.5)
            actual_distance = np.abs(current_belief - 0.5)

            is_validation_response = (pending_friction == 1.0) & (actual_distance > expected_distance + 0.01)
            ...
            new_pending_friction = np.where(
                pending_friction == 1.0,
                0.0,
                new_pending_friction
            )

            pending_friction = new_pending_friction
            pending_expected_belief = new_pending_expected
        ...
        # === TYPE-DEPENDENT RESPONSE (from run_sim_heterogeneous_types) ===
        # θ_V resists friction by blending back toward Bayesian update
        theta_v_prior = (1 - resistance_strength) * friction_applied_prior + resistance_strength * bayes_prior
        ...
        new_carry = (
            ...
            commit_beliefs, commit_count, type_confidence,
            friction_response_sum, friction_response_count,
            friction, current_belief, pending_friction, pending_expected_belief,
            user_type, cumulative_work
        )
    ...
    def run_one_sim(seed):
        # Draw user type: 0 = θ_G (Growth), 1 = θ_V (Validation)
        key = jax.random.key(seed)
        key, key_type = jax.random.split(key)
        user_type = jax.random.bernoulli(key_type, p_validation).astype(np.float32)
        ...
        init_carry = (
            ...
            0.0,    # last_friction
            init_belief,  # last_belief
            0.0,    # pending_friction
            0.5,    # pending_expected_belief
            user_type,
            0.0     # cumulative_work
        )
        ...
        priors, frictions, type_confs, checkouts, works = outputs
        return (priors, frictions, type_confs, checkouts, works, user_type)
    ...
    priors = results[0]
    frictions = results[1]
    type_confidences = results[2]
    checkouts = results[3]
    works = results[4]
    user_types = results[5]

    return priors, frictions, type_confidences, checkouts, works, user_types
...
def analyze_versioning_heterogeneous_results(
    results: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
    type_confidence_threshold: float = 0.7,
    work_threshold: float = 0.5
) -> dict:
    """
    Analyze results from run_sim_versioning_heterogeneous.

    Args:
        results: (priors, frictions, type_confidences, checkouts, works, user_types)
        type_confidence_threshold: Threshold for classifying as θ_V
        work_threshold: Epistemic work threshold for work-based classification

    Returns:
        Statistics on type separation and detection accuracy using BOTH:
        - work_based_classification: Uses epistemic work (comparable to paper)
        - system_classification: Uses type_confidence (what system believes)
    """
    priors, frictions, type_confidences, checkouts, works, user_types = results

    # Extract final beliefs and type confidence
    final_beliefs = extract_final_beliefs_batch(priors)
    final_type_conf = type_confidences[:, -1]
    ...
    # ========================================================================
    # WORK-BASED CLASSIFICATION (comparable to paper)
    # ========================================================================
    # High work = θ_G (engaged with friction), Low work = θ_V (resisted)
    work_detected_growth = total_work > work_threshold
    work_detected_validation = ~work_detected_growth

    work_tp_g = float((work_detected_growth & true_growth).sum())
    work_tp_v = float((work_detected_validation & true_validation).sum())
    work_fp_g = float((work_detected_growth & true_validation).sum())
    work_fp_v = float((work_detected_validation & true_growth).sum())

    work_recall_g = work_tp_g / n_growth if n_growth > 0 else 0.0
    work_recall_v = work_tp_v / n_validation if n_validation > 0 else 0.0
    work_precision_g = work_tp_g / (work_tp_g + work_fp_g) if (work_tp_g + work_fp_g) > 0 else 0.0
    work_precision_v = work_tp_v / (work_tp_v + work_fp_v) if (work_tp_v + work_fp_v) > 0 else 0.0
    work_overall_accuracy = (work_tp_g + work_tp_v) / (n_growth + n_validation)

    mean_work_g = float(total_work[true_growth].mean()) if n_growth > 0 else 0.0
    mean_work_v = float(total_work[true_validation].mean()) if n_validation > 0 else 0.0

    # ========================================================================
    # SYSTEM CLASSIFICATION (type_confidence-based, what system believes)
    # ========================================================================
    sys_detected_validation = final_type_conf > type_confidence_threshold
    sys_detected_growth = ~sys_detected_validation

    sys_tp_g = float((sys_detected_growth & true_growth).sum())
    sys_tp_v = float((sys_detected_validation & true_validation).sum())
    sys_fp_g = float((sys_detected_growth & true_validation).sum())
    sys_fp_v = float((sys_detected_validation & true_growth).sum())

    sys_recall_g = sys_tp_g / n_growth if n_growth > 0 else 0.0
    sys_recall_v = sys_tp_v / n_validation if n_validation > 0 else 0.0
    sys_precision_g = sys_tp_g / (sys_tp_g + sys_fp_g) if (sys_tp_g + sys_fp_g) > 0 else 0.0
    sys_precision_v = sys_tp_v / (sys_tp_v + sys_fp_v) if (sys_tp_v + sys_fp_v) > 0 else 0.0
    sys_overall_accuracy = (sys_tp_g + sys_tp_v) / (n_growth + n_validation)
    ...
    mean_checkouts_g = float(total_checkouts[true_growth].mean()) if n_growth > 0 else 0.0
    mean_checkouts_v = float(total_checkouts[true_validation].mean()) if n_validation > 0 else 0.0

    stats = {
        'n_growth': int(n_growth),
        'n_validation': int(n_validation),

        # Work-based classification (PRIMARY, comparable to paper)
        'recall_growth': work_recall_g,
        'recall_validation': work_recall_v,
        'precision_growth': work_precision_g,
        'precision_validation': work_precision_v,
        'overall_detection_accuracy': work_overall_accuracy,
        'mean_work_growth': mean_work_g,
        'mean_work_validation': mean_work_v,
        'work_separation': mean_work_g - mean_work_v,

        # System classification (SECONDARY, what system believes in real-time)
        'system_recall_growth': sys_recall_g,
        'system_recall_validation': sys_recall_v,
        'system_precision_growth': sys_precision_g,
        'system_precision_validation': sys_precision_v,
        'system_overall_accuracy': sys_overall_accuracy,
        ...
        'mean_final_belief_growth': float(final_beliefs[true_growth].mean()) if n_growth > 0 else 0.0,
        'mean_final_belief_validation': float(final_beliefs[true_validation].mean()) if n_validation > 0 else 0.0,
        'mean_checkouts_growth': mean_checkouts_g,
        'mean_checkouts_validation': mean_checkouts_v,
        'mean_checkouts_per_sim': float(total_checkouts.mean()),
        'mean_final_type_confidence': float(final_type_conf.mean()),
    }
